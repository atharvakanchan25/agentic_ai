...
Orchestrator - Coordinates all agents with:
  - SLIM envelope + HMAC auth token on every agent call
  - OpenTelemetry root span per pipeline run
...
class AgentOrchestrator:
    ...
    async def _call(self, agent, method: str, params: dict) -> dict:
        """
        Secured agent call:
        1. Builds a SLIM envelope (signed, versioned, TTL-checked)
        2. Attaches an HMAC auth token
        3. Calls agent.process_request which validates both before executing
        """
        from src.slim.protocol import create_envelope
        from src.security.agent_security import sign_agent_call
        from src.observability.telemetry import get_current_trace_id, get_current_span_id

        run_id = self._run_memory.get("run_id", "")
        ...
        token = sign_agent_call(agent.agent_name, method, run_id)
    ...
    async def _schedule_department(self, dept_input: dict, plan: dict) -> dict:
        ...
        if conflicts:
            resolution = await self._call(self.conflict_agent, "resolve_conflicts", {
                "timetable": timetable, "conflicts": conflicts,
    ...
    async def run(self, input_data: dict[str, Any], run_id: str = None,
        ...
        from src.observability.telemetry import pipeline_span

        with pipeline_span(run_id):
            ...
            if validation.get("status") == "invalid":
                return {"status": "failed", "stage": "validation",
                        "errors": validation.get("errors", []), "pipeline_log": self.pipeline_log}
            ...
            if completeness.get("status") == "incomplete":
                return {"status": "failed", "stage": "completeness",
                        "missing": completeness.get("missing_entities", []), "pipeline_log": self.pipeline_log}

            constraints = await self._call(self.validation_agent, "verify_constraints", input_data)
            if constraints.get("status") == "unsatisfiable":
                return {"status": "failed", "stage": "constraints",
                        "issues": constraints.get("constraint_issues", []), "pipeline_log": self.pipeline_log}
            ...
            if plan.get("parallel_depts") and len(input_data.get("divisions", [])) > 4:
                ...
                for dept_id, divs in dept_groups.items():
                    ...
                    tasks.append(self._schedule_department({**input_data, "divisions": divs, "subjects": dept_subjects}, plan))

                dept_results = await asyncio.gather(*tasks, return_exceptions=True)
                timetable, solve_time, quality_metrics = [], 0.0, {}
                ...
                optimization = {
                    "status": optimization_status, "solver_status": "parallel_complete",
                    "solver_mode": plan.get("solver_mode"), "timetable": timetable,
                    "solve_time": solve_time, "quality_metrics": quality_metrics,
                }
            else:
                optimization = await self._call(
                    self.optimization_agent, "optimize_timetable",
            ...
            if hitl_enabled and "optimization" in plan.get("hitl_after", []):
                ...
                raise HumanApprovalRequired(run_id, "post_optimization", {
                    "message": f"Optimization complete ({optimization.get('solver_status')}). Approve to continue.",
